package person.liwy.common.json;
...
public class JacksonUtil {
    ...
    static {
        try {
            ...
            log.error("jackson config error {}", e.getLocalizedMessage());
        }
    }
    ...
    public static <V> void toCsvFile(String path, String separator, V v) {
        try (Writer writer = new FileWriter(new File(path), true)) {
            CsvSchema schema;
            schema = csvMapper.schemaFor(v.getClass()).withHeader().withColumnSeparator(separator.charAt(0));
